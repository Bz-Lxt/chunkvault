...
BODY="WIPE-keep-this-payload-across-chunk-windows!!"
PUT="$(curl -sS -X PUT "http://127.0.0.1:$PORT/v1/blobs?name=w" --data-binary "$BODY")"
DIGEST="$(printf '%s' "$PUT" | json_get "['digest']")"
CODE="$(curl -sS -o "$WORKDIR/got" -w "%{http_code}" "http://127.0.0.1:$PORT/v1/blobs/$DIGEST")"
ACTUAL="GET $CODE $(wc -c < "$WORKDIR/got" | tr -d ' ')B"

echo "[EXPECT] GET 200 original WIPE payload"
echo "[ACTUAL] $ACTUAL"
