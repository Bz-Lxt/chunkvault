...
PUT="$(curl -sS -X PUT "http://127.0.0.1:$PORT/v1/blobs?name=n" --data-binary "NILHANDLE")"
DIGEST="$(printf '%s' "$PUT" | json_get "['digest']")"
CODE="$(curl -sS -o "$WORKDIR/got" -w "%{http_code}" "http://127.0.0.1:$PORT/v1/blobs/$DIGEST")"
ACTUAL="GET $CODE"

echo "[EXPECT] GET 200 body NILHANDLE"
echo "[ACTUAL] $ACTUAL"
if [[ "$CODE" == "200" ]] && [[ "$(cat "$WORKDIR/got")" == "NILHANDLE" ]]; then
  exit 0
fi
exit 1
