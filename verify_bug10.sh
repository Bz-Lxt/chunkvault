...
PUT="$(curl -sS -X PUT "http://127.0.0.1:$PORT/v1/blobs?name=k" --data-binary "keep-after-checkpoint")"
DIGEST="$(printf '%s' "$PUT" | json_get "['digest']")"
curl -sS -X POST "http://127.0.0.1:$PORT/v1/checkpoint" >/dev/null
CODE="$(curl -sS -o "$WORKDIR/got" -w "%{http_code}" "http://127.0.0.1:$PORT/v1/blobs/$DIGEST")"
ACTUAL="GET $CODE"

echo "[EXPECT] GET 200 after checkpoint"
echo "[ACTUAL] $ACTUAL"
if [[ "$CODE" == "200" ]] && [[ "$(cat "$WORKDIR/got")" == "keep-after-checkpoint" ]]; then
  exit 0
fi
exit 1
