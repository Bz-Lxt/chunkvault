...
PUT="$(curl -sS -X PUT "http://127.0.0.1:$PORT/v1/blobs?name=u" --data-binary "unique-only-once-payload")"
DIGEST="$(printf '%s' "$PUT" | json_get "['digest']")"
curl -sS -X DELETE "http://127.0.0.1:$PORT/v1/blobs/$DIGEST" >/dev/null
curl -sS -X POST "http://127.0.0.1:$PORT/v1/gc" >/dev/null
STATS="$(curl -sS "http://127.0.0.1:$PORT/v1/stats")"
CHUNKS="$(printf '%s' "$STATS" | json_get "['chunks']")"
ACTUAL="chunks=$CHUNKS"

echo "[EXPECT] chunks=0 after unlink+GC"
echo "[ACTUAL] $ACTUAL"
if [[ "$CHUNKS" == "0" ]]; then
  exit 0
fi
